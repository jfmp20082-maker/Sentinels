<!-- This is a Kanban Board file created with MD Kanban extension -->
<!-- GitHub: https://github.com/jebakumarj/md-kanban -->
<!-- VS Code Extension: Search "MD Kanban" in the extension store (ID: jeddak.md-kanban) -->

# Sentinel Board

## Backlog

#### Requisitos
<!-- id: task-1789765199446-0 -->
Añadir requisitos del sistema para poderlo ejectuar

#### Estructura de proyecto
<!-- id: task-1789765368261-4 -->
Corregir estructura de carpetas y archivos

src/ #Codigo del proyecto
docs/ #Documentacion
test/ #Pruebas
README.md

#### Añadir las nuevas tecnologias al repo
<!-- id: task-1789770258829-37 -->
Añadir 
Docker
Postgresql/SupaBase

#### Documentacion tecnica
<!-- id: task-1789770442707-66 -->
Realizar la documentacion tecnica de todo el codigo fuente

## Ready

## In Progress

#### Diagnistico del repositorio
<!-- id: task-1789770395833-50 -->
Chequeo del estado del repositorio y ver areas de mejora o implementacion

## Review

#### Correccion del README.md
<!-- id: task-1789765388616-9 -->

## Done
